...
double suma=0;  /* summation of normal mode sampling of acoustic branch */						/*argument=k*dist;
...
double disp=0; /* displacement of two atoms for dipole moment*/
double dum=0; /* pause variable*/
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~inputs~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
/*construct inverse square root for atoms*/
double MassInvB=sqrt(1/MassPt);
double MassInvA=sqrt(1/MassX);
...
gsl_vector *evec_i = gsl_vector_alloc (order);
...
FILE* f30 = fopen("eigenvalue_check.txt","w+");
FILE* f20 = fopen("eigenvector_check.txt","w+");
for (i=0; i<4*N; i++)
{
 ...
 fprintf (f30,"%lf\n",wave);
 gsl_vector_fprintf (f20, &evec_i.vector,"%lf");
  fprintf (f20,"\n");
 }
 fclose(f30);
 fclose(f20);
...
FILE* f10 = fopen("K0_displacements_Acc.txt", "w+");
n=0;
for (i = 0; i < N; i++)
      {
		/* get ith eigenvalue and eigenvector*/
        evalue_i = gsl_vector_get (evalue, i);
        gsl_matrix_get_col(evec_i,evector,i);
		...
		if(wave>1)
			{
				...
				/* difference term for dipole moment*/
				atomB=gsl_matrix_get(evector,1,i);
				atomB=atomB*MassInvB;
				atomA=gsl_matrix_get(evector,0,i);
				atomA=atomA*MassInvA;
				disp=atomB-atomA;
			...
			if (-0.002<k && k<0.002)
			{
			gsl_vector_fprintf(f10,evec_i, "%lf");
			fprintf(f10,"\n\n");
			}
			
		printf ("%i\n\n",n);
		printf ("eigenvalue = %g\n", evalue_i);
		printf ("max displacement of two atoms for this frequency %g\n", disp);
...
FILE* f11 = fopen("K0_displacements_Nu3.txt", "w+");
n=0;
for (i = N; i < 2*N; i++)
      {
		/* get ith eigenvalue and eigenvector*/
      ...
      gsl_matrix_get_col(evec_i,evector,i);
		...
		if (-0.002<k && k<0.002)
			{
			gsl_vector_fprintf(f11,evec_i, "%lf");
			fprintf(f11,"\n\n");
			}
		printf ("%i\n\n",n);
		printf ("eigenvalue = %g\n", evalue_i);
...
FILE* f12 = fopen("K0_displacements_Nu1.txt", "w+");
n=0;
for (i = 2*N; i < 3*N; i++)
      {
		/* get ith eigenvalue and eigenvector*/
  ...
  gsl_matrix_get_col(evec_i,evector,i);
		...
		if (-0.002<k && k<0.002)
			{
			gsl_vector_fprintf(f12,evec_i, "%lf");
			fprintf(f12,"\n\n");
			}
		printf ("%i\n\n",n);
		printf ("eigenvalue = %g\n", evalue_i);
...
FILE* f13 = fopen("K0_displacements_Nu2.txt", "w+");
n=0;
for (i = 3*N; i < order; i++)
      {
		/* get ith eigenvalue and eigenvector*/
    ...
    gsl_matrix_get_col(evec_i,evector,i);
		...
		if (-0.002<k && k<0.002)
			{
			gsl_vector_fprintf(f13,evec_i, "%lf");
			fprintf(f13,"\n\n");
			}
		printf ("%i\n\n",n);
		printf ("eigenvalue = %g\n", evalue_i);
